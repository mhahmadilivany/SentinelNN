...
def vulnerability_values_space_act(inputs, weights, device):
    deviations = torch.zeros((32, torch.numel(inputs)), device=device)
    resolution = 10

    #obtaining added errors to conv's output with SEU in activations
    inputs_cp = torch.clone(inputs)
    for bit_loc in range(32):
        inputs_bin = binary_converter.float2bit(inputs_cp.flatten(), device=device)
        inputs_bin[:, bit_loc] = torch.logical_xor(inputs_bin[:, bit_loc], torch.ones(inputs_bin[:, bit_loc].size(), device=device)).float()
        dev = binary_converter.bit2float(inputs_bin, device=device) - inputs.flatten()
        dev = dev.reshape(inputs.size()) * weights
        dev = torch.nan_to_num(dev, nan=10000)
        deviations[bit_loc] = dev.flatten()
        
        inputs_cp = torch.clone(inputs)

    dict_errors_dist_act = vulnerability_values_grouping(torch.sort(deviations.flatten())[0], resolution)

    errors_dist_act = torch.zeros(len(dict_errors_dist_act), device=device)
    ind = 0
    for i in dict_errors_dist_act:
        errors_dist_act[ind] = dict_errors_dist_act[i]
        ind += 1
    ...
    del dev
    del deviations
    del dict_errors_dist_act

    return errors_dist_act
